...
function EU = SimpleCalcExpectedUtility(I)
  ...
  variablesToEliminate = findIndicesToEliminateFromFactors(F, U);
  factorsList = VariableElimination(F, variablesToEliminate);
  jointProb = getJointProbabilityFromListOfFactors(factorsList);
  EU = computeExpectedUtilityFromJointProb(jointProb, U);
end
