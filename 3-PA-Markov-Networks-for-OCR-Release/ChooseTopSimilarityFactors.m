...
if (length(allFactors) <= F)
    ...
    indexToFindSimilarity = AssignmentToIndex([1, 1], allFactors(1).card);

    allFactors = sortFactorsBySimilarity(allFactors, indexToFindSimilarity);
    factors = allFactors(1:F);

    function factors = sortFactorsBySimilarity(factors, indexToFindSimilarity)
        valuesTable = horzcat(factors(:).val);
        simTable = valuesTable(indexToFindSimilarity, :);
        [~, indices] = sort(simTable, 'descend');
