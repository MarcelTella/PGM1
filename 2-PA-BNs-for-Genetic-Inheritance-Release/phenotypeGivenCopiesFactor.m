...
for i=1:length(phenotypeFactor.val)
    ...
    thisIdxAlpha = allelesToGenotypes(assignment(2),assignment(3));
	if assignment(1) == 1 % If phenotype is 1.
		phenotypeFactor.val(i) = alphaList(thisIdxAlpha);
	else	% if 2, if phenotype is 2
		phenotypeFactor.val(i) = 1 - alphaList(thisIdxAlpha);
	end
end
