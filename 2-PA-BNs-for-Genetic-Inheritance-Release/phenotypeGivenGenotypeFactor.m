...
nGenCombinations = length(alphaList);
phenotypeFactor.card = [2, nGenCombinations];

vectorProbabilities = zeros(2*nGenCombinations, 1);
for i=1:nGenCombinations
    vectorProbabilities(i*2-1) = alphaList(i);
    vectorProbabilities(i*2) = 1-alphaList(i);
end

phenotypeFactor.val = vectorProbabilities;
