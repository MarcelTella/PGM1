...
genotypeFactorParShouldBe = struct('var', [3,1,2], 'card', [3,3,3], 'val', [1,0,0,0.5,0.5,0,0,1,0,0.5,0.5,0,0.25,0.5,0.25,0,0.5,0.5,0,1,0,0,0.5,0.5,0,0,1]); % Comment out this line for testing
...
assert(isequal(genotypeFactorParShouldBe.val, genotypeFactorPar.val));
...
sampleFactorListObtained = constructGeneticNetwork(pedigree, alleleFreqs, alphaList);
...
phenotypeFactorParShouldBe = phenotypeGivenCopiesFactor(alphaListThree, numAllelesThree, genotypeVarMotherCopy, genotypeVarFatherCopy, phenotypeVar);
...
sampleFactorListDecoupled
sampleFactorListDecoupledRes = constructDecoupledGeneticNetwork(pedigree, alleleFreqsThree, alphaListThree);
