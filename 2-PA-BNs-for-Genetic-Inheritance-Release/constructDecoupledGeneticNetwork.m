...
% not matter.
factorList(numVariables) = struct('var', [], 'card', [], 'val', []);
numAlleles = length(alleleFreqs); % Number of alleles

% Genotype factors
for i=1:numPeople
    varNumGen1 = i;
    varNumGen2 = i+numPeople;
    thisParents = pedigree.parents(i, :);
    
    if noParents(thisParents)
        factorList(i*2-1) = childCopyGivenFreqsFactor(alleleFreqs, varNumGen1);
        factorList(i*2) = childCopyGivenFreqsFactor(alleleFreqs, varNumGen2);
    else    
        factorList(i*2-1) = childCopyGivenParentalsFactor(numAlleles, varNumGen1, thisParents(1), thisParents(1)+numPeople);
        factorList(i*2) = childCopyGivenParentalsFactor(numAlleles, varNumGen2, thisParents(2), thisParents(2)+numPeople);
    end
...
for i=1:numPeople
    varNumGenCopy1 = i;
    varNumGenCopy2 = i+numPeople;
    varNumPhen = 2*numPeople+i;
    factorList(i+2*numPeople) = phenotypeGivenCopiesFactor(alphaList, numAlleles, varNumGenCopy1, varNumGenCopy2, varNumPhen);
end

function bool = noParents(thisParents)
    bool = isequal(thisParents, [0, 0]);
end
