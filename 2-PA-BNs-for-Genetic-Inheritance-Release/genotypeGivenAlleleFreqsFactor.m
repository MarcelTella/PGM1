...
for i=1:numGenotypes
   thisGenotypeIdx = i; 
   rowThisAlleles = genotypesToAlleles(thisGenotypeIdx, :);
   alleleI = rowThisAlleles(1);
   alleleJ = rowThisAlleles(2);
   
   % Since the prob. of an allele is independent, we multiply them.
   genotypeFactor.val(thisGenotypeIdx) = ...
       alleleFreqs(alleleI) * alleleFreqs(alleleJ);
   
   % if heterozygotes, we have to account twice, since we discounted the
   % value. e.g. from Aa, we only consider Aa, but not aA.
   % if we now had Aa and Bb and Cc, we would also have to double this
   % prob.
   if alleleFreqs(alleleI) ~= alleleFreqs(alleleJ)
       genotypeFactor.val(thisGenotypeIdx) = 2*genotypeFactor.val(thisGenotypeIdx);
   end
end
