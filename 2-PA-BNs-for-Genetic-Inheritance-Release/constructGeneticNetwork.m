...
for i=1:numPeople
    ...
    if noParents(thisParents)
        factorList(i+numPeople) = genotypeGivenAlleleFreqsFactor(alleleFreqs, varNumGen);
    else    
        factorList(i+numPeople) = genotypeGivenParentsGenotypesFactor(numAlleles, varNumGen, thisParents(1), thisParents(2));
    end
...
for i=1:numPeople
    varNumGen = i;
    varNumPhen = numPeople+i;
    factorList(i) = phenotypeGivenGenotypeFactor(alphaList, varNumGen, varNumPhen);
end
